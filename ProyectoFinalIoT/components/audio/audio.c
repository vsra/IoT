...
// esta es la lista de canciones, en el código original era un const, pero lo borramos
// para poder agregar las canciones que queremos.

char audio_list[AUDIO_MAX_PLAY_LIST][64] = {
    "/spiffs/lemon_tree_8k(1).mp3",
   ...
   // uint32_t touch_num = 0;
    ...
    while (1) {
        ...
        if (xQueueReceive(command_queue, &command, portMAX_DELAY) == pdPASS) {
            ...
            // Procesar el comando leído
            ESP_LOGI(TAG, "comando leido: %s", command);
...
void loadQueue(char com)
{
    if (xQueueSend(command_queue, &com, portMAX_DELAY) != pdPASS) {
        ESP_LOGE(TAG, "Failed to send command to queue");
    }else{
        ESP_LOGI(TAG, "Command successfully sent to queue");
    }
}
...
int audio_init(led_strip_t *strip)
{
    ...
    //command_queue = xQueueCreate(10, sizeof(event_t));
